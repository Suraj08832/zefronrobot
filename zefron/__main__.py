...
from zefron.modules.no_sql import get_served_chats,get_served_users
from zefron import (
    BOT_NAME,
    BOT_USERNAME,
    LOGGER,
...
from zefron.modules.helper_funcs.chat_status import is_user_admin
from zefron.modules.helper_funcs.misc import paginate_modules
...
for module_name in ALL_MODULES:
    imported_module = importlib.import_module("zefron.modules." + module_name)
...
def zefron_Main_Callback(update: Update, context: CallbackContext):
    query = update.callback_query
...
Yᴏᴜ ᴄᴀɴ ʜᴇʟᴘ ᴛʜᴇ ᴅᴇᴠᴇʟᴏᴘᴍᴇɴᴛ ᴡɪᴛʜ ᴅᴏɴᴀᴛɪᴏɴs""",parse_mode=ParseMode.MARKDOWN,
        ...
        InlineKeyboardButton(text="🖥️", callback_data="Main_help"),
                    ...
                    [
                        InlineKeyboardButton(text="• Dᴏɴᴀᴛᴇ •", url="https://bots_update_all/7"),InlineKeyboardButton(text="• sᴜᴘᴘᴏʀᴛ •", callback_data="mukesh_support")
                    ]
                ]
            ),
            )  
...
def donate(update: Update, context: CallbackContext):
    ...
    if chat.type == "private":
        ...
        if OWNER_ID != 5935608297:
            update.effective_message.reply_text(
                f"» ᴛʜᴇ ᴅᴇᴠᴇʟᴏᴩᴇʀ ᴏғ {dispatcher.bot.first_name} sᴏᴜʀᴄᴇ ᴄᴏᴅᴇ ɪs [ɢɪᴛʜᴜʙ](https://bots_update_all/zefron)"
...
""",reply_markup=x,
    ...
    mukeshrobot_main_handler = CallbackQueryHandler(
        zefron_Main_Callback, pattern=r".*_help",run_async=True)
