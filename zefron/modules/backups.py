...
# from zefron.modules.sql import warns_sql as warnssql
import zefron.modules.sql.blacklist_sql as blacklistsql

# from zefron.modules.sql import cust_filters_sql as filtersql
# import zefron.modules.sql.welcome_sql as welcsql
import zefron.modules.sql.locks_sql as locksql
import zefron.modules.sql.notes_sql as sql

# from zefron.modules.rules import get_rules
import zefron.modules.sql.rules_sql as rulessql
from zefron import EVENT_LOGS,OWNER_ID, SUPPORT_CHAT, dispatcher
from zefron.__main__ import DATA_IMPORT
from zefron.modules.connection import connected
from zefron.modules.helper_funcs.alternate import typing_action
from zefron.modules.helper_funcs.chat_status import user_admin
from zefron.modules.sql import disable_sql as disabledsql
...
def export_data(update, context):
    ...
    with open("zefron{}backup.json".format(chat_id), "w") as f:
    ...
    context.bot.sendDocument(
        current_chat_id,
        document=open("zefron{}backup.json".format(chat_id), "rb"),
        caption="📤*Successfully Exported backup:*\nChat: `{}`\nChat ID: `{}`\nOn: `{}`\n\nNote: This `zefron-Backup` was specially made for notes 📚.".format(
            chat.title, chat_id, tgl
        ),
        timeout=360,
    ...
    os.remove("zefron{}backup.json".format(chat_id))  # Cleaning file
