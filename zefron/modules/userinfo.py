...
from zefron.modules.alive import Mukesh
import zefron.modules.sql.userinfo_sql as sql
from zefron import (
    DEMONS,
    DEV_USERS,
    DRAGONS,
...
from zefron.__main__ import STATS, TOKEN, USER_INFO
from zefron.modules.disable import DisableAbleCommandHandler
from zefron.modules.helper_funcs.chat_status import sudo_plus
from zefron.modules.helper_funcs.extraction import extract_user
from zefron.modules.sql.global_bans_sql import is_user_gbanned
from zefron.modules.sql.users_sql import get_user_num_chats
...
def info(update: Update, context: CallbackContext):
    ...
    if user.id == OWNER_ID:
        ...
        text += "\n\nᴛʜɪs ᴜsᴇʀ ɪs ᴀ ᴍᴇᴍʙᴇʀ ᴏғ <b>suraj ᴀssᴏᴄɪᴀᴛɪᴏɴ</b>.\n"
        disaster_level_present = True
    ...
    if disaster_level_present:
        text += ' \n[<a href="https://bots_update_all/26">ᴄʟɪᴄᴋ ʜᴇʀᴇ ᴛᴏ ᴋɴᴏᴡ ᴡʜᴀᴛ ɪs ᴅɪsᴀsᴛᴇʀ ʟᴇᴠᴇʟs.</a>]'.format(
            bot.username
        )
            ...
            message.reply_photo(
                ...
                reply_markup=InlineKeyboardMarkup(
                    [
                        [
                            InlineKeyboardButton(
                                "ʜᴇᴀʟᴛʜ", url=f"https://bots_update_all/90"
                            ),
                            InlineKeyboardButton(
                                "Dɪsᴀsᴛᴇʀ", url="https://bots_update_all/26"
                            ),
                        ],
                        [
                            InlineKeyboardButton(
            ...
            message.reply_text(
                ...
                reply_markup=InlineKeyboardMarkup(
                    [
                        [
                            InlineKeyboardButton(
                                "ʜᴇᴀʟᴛʜ", url="https://bots_update_all/90"
                            ),
                            InlineKeyboardButton(
                                "ᴅɪsᴀsᴛᴇʀ", url="https://bots_update_all/26"
                            ),
                        ],
                        [
                            InlineKeyboardButton(
...
def set_about_bio(update: Update, context: CallbackContext):
    ...
    if message.reply_to_message:
        ...
        if user_id == bot.id and sender_id not in DEV_USERS:
            message.reply_text(
                "ᴜᴍᴍ... ʏᴇᴀʜ, ɪ ᴏɴʟʏ ᴛʀᴜsᴛ suraj ᴀssᴏᴄɪᴀᴛɪᴏᴍ ᴛᴏ sᴇᴛ ᴍʏ ʙɪᴏ."
            )
            return
